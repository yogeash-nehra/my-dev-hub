...
type AgentId = 'analyst' | 'risk' | 'manager'
...
// ── Storage helpers — KV primary, localStorage fallback ───────────────────────
...
async function kvLoad(): Promise<Position[] | null> {
  try {
    ...
    if (data.positions === null) return null   // KV not configured
...
async function kvSave(positions: Position[]): Promise<void> {
  try {
    ...
    // KV save failed — localStorage already updated as fallback
  }
}
...
async function loadPositions(): Promise<Position[]> {
  ...
  if (remote !== null) {
    lsSave(remote)   // keep localStorage in sync
    return remote
  }
  return lsLoad()   // KV not configured — use localStorage
}

async function savePositions(positions: Position[]): Promise<void> {
  lsSave(positions)    // always update localStorage immediately
...
export default function PortfolioPage() {
  ...
  const [collapsed,     setCollapsed]     = useState<Set<AgentId>>(new Set())
  ...
  // Load positions — KV primary, localStorage fallback
  useEffect(() => {
  ...
  // Fetch live quotes whenever positions change
  const refreshQuotes = useCallback(async (pos: Position[]) => {
    ...
    try {
      ...
      // silent — positions still show with avg cost
    } finally {
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              ...
              <p className="text-slate-400 text-sm max-w-xl">
                Track your positions with live prices. Run AI analysis to get per-position signals and
                portfolio-level stance from three specialized agents.
              </p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
        ...
        {positions.length === 0 ? (
          <div className="bg-[#080D14] border border-white/5 rounded-xl p-12 text-center">
            ...
            <p className="text-slate-400 text-sm">No positions yet. Add your first holding above.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {positionsWithPrices.map(pos => {
                    ...
                    const signal = result?.signals[pos.ticker]
                    const q = quotes[pos.ticker]
                    return (
                      <tr key={pos.id} className="border-b border-white/3 hover:bg-white/2 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right">
                          <EditableNumber
                            value={pos.shares}
                            onSave={v => updateShares(pos.id, v)}
              ...
              <div className="flex items-center justify-between px-4 py-2.5 bg-[#0A0F18] border-b border-white/8">
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono text-xs text-slate-500">
                    {running ? `Step ${currentStep}/3 — ${stepLabel}` : result ? 'analysis complete' : 'agent terminal'}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  {running && (
...
// ── EditableNumber — click to edit inline ──────────────────────────────────────
