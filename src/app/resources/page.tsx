...
function ResourceCard({ resource }: { resource: Resource }) {
  return (
    <Link href={`/resources/${resource.slug}`} style={{ textDecoration: 'none' }}>
      ...
      >
        <div style={{ fontSize: 15, fontWeight: 650, color: '#F1F5F9', marginBottom: 6 }}>
          {resource.title}
        </div>
...
function Section({ title, resources }: { title: string; resources: Resource[] }) {
  ...
  return (
    <div style={{ marginBottom: 36 }}>
      <h2 style={{ fontSize: 13, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#64748B', marginBottom: 14 }}>
        {title}
      </h2>
    ...
    <main style={{ background: '#030712', minHeight: 'calc(100vh - 48px)', padding: '48px 24px 80px' }}>
      <div style={{ maxWidth: 900, margin: '0 auto' }}>
        <div style={{ marginBottom: 40 }}>
          <h1 style={{ fontSize: 36, fontWeight: 750, color: '#F8FAFC', margin: '0 0 14px', letterSpacing: '-0.025em' }}>
            Resources
          </h1>
